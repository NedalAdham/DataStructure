...
using namespace std ;
/*
   PRISM ALGO to get MST
   go to every neighbour vertex which is not visited
   update node weight if edge weight of neighbour is less than it
 vertex        parent             weight
   ...
   minimum heap****
    ...
    for each vertex v
    parent [v] = -1 , wt[v] = MAX_INT , visited [v] = false
    wt[s]=0
    creat min heap of all vertex
    while (Q!= )
            u = getmin(q)
            visited[u] = true
            for each neighbour v of u
                if not visted and dist[u,v] = dist[u,v]
                parent[v] =u
 */
